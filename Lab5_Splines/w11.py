...
def hermite(points, t=30):
    ...
    ta = np.array([
        [t**3, t**2, t, 1] for t in [0, 1]
        ])
    
    tb = np.array([
        [3*(t**2), 2*t, 1, 0] for t in [0, 1]
        ])
    
    dc = np.vstack([ta, tb])
    mh = np.array(np.linalg.inv(dc))
    fa = np.array(dc@mh@permute_mat@points)
