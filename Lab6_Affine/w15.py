...
def get_affine_transform(src_points, tgt_points):
    ...
    l = len(src_points)
    my = np.transpose(src_points)
    D = 1.0 / np.linalg.det(my)
    entry = lambda r,d: np.linalg.det(np.delete(np.vstack([r, my]), (d+1), axis=0))
    M = [[(-1)**i * D * entry(R, i) for i in range(l)] for R in np.transpose(tgt_points)]
    A, t= np.hsplit(np.array(M), [l])
...
perspective_target_points_clicked = [np.array([0.1, 0.1, 1]), np.array([0.1, 0.4, 1]), np.array([0.4, 0.4, 1]),  np.array([0.4, 0.1, 1])]
